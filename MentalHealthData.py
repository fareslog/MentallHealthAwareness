...
# Set the number of rows as requested (Increased to 11000)
N_ROWS = 11000
...
# Age: Adjusted scale slightly for a wider distribution
...
# Note: Increased the upper bound for self-employed to introduce more high-end outliers naturally
...
# Sleep hours: Increased scale for wider variance
sleep_hours = np.clip(np.random.normal(6.5, 2.0, size=N_ROWS), 1.0, 12.0)
exercise_freq = np.random.randint(0, 10, size=N_ROWS) # Days per week (allowing up to 9 for data errors/intensity)
social_support = np.random.choice(['Very Low', 'Low', 'Medium', 'High', 'MISSING'], size=N_ROWS, p=[0.1, 0.25, 0.35, 0.2, 0.1])
gad7_scores = np.random.randint(0, 22, size=N_ROWS)
phq9_scores = np.random.randint(0, 28, size=N_ROWS)
stress_levels = np.random.randint(0, 10, size=N_ROWS)
...
# --- 5. Introduce EXTREME MESSINESS (NaNs, Outliers, Illogical Data) ---

# 5.1 Increase NaN (missing values) to 15% across selected columns
...
# 5.2 Add extreme Outliers: Very high work hours and very low sleep
# Increase sample size for outliers to 20 rows
outlier_indices_1 = data.sample(n=20, random_state=101).index
data.loc[outlier_indices_1, 'Work_Hours_Week'] = np.random.randint(140, 180, size=20) # Extreme work hours
data.loc[outlier_indices_1, 'Sleep_Hours_Night'] = np.random.uniform(0.5, 2.5, size=20) # Very low sleep
data.loc[outlier_indices_1, 'Stress_Level_Scale'] = 9 # Max stress

# Outlier 3: Illogical sleep hours (negative or zero)
illogical_indices = data.sample(n=10, random_state=103).index
data.loc[illogical_indices, 'Sleep_Hours_Night'] = np.random.uniform(-10.0, 0, size=10) # Negative/Zero sleep hours

# Outlier 4: Illogical exercise frequency (much too high)
illogical_freq_indices = data.sample(n=5, random_state=104).index
data.loc[illogical_freq_indices, 'Exercise_Freq_Week'] = np.random.choice([50, 99], size=5) # Typo errors in frequency

# 5.3 Add zero/empty strings and placeholder values
data.loc[data.sample(frac=0.07, random_state=105).index, 'Education_Level'] = 'N/A' # Placeholder string
data.loc[data.sample(frac=0.05, random_state=106).index, 'Gender'] = 'NA' # Placeholder string in Gender

# --- 6. Save the CSV file ---
output_filename = 'HealthMind_Mental_Health_Data_11000_Extreme_Messy.csv'
data.to_csv(output_filename, index=False)

print(f"✅ Successfully generated {N_ROWS} rows of SYNTHETIC, EXTREME MESSY data.")
print(f"File saved as '{output_filename}'. You can now download and use this file for rigorous preprocessing.")
